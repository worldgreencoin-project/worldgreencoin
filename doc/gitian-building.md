...
*Setup instructions for a gitian build of WorldGreenCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the WorldGreenCoin
...
- [Building WorldGreenCoin](#building-worldgreencoin)
...
Clone the git repositories for worldgreencoin and gitian,
...
In [doc/release-process.md](release-process.md) in the worldgreencoin repository under 'Fetch and build inputs'.
...
Building WorldGreenCoin
----------------

To build WorldGreenCoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the worldgreencoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/worldgreencoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/worldgreencoin-project/worldgreencoin
...
For example:
```bash
URL=https://github.com/user/worldgreencoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit worldgreencoin=${COMMIT} --url worldgreencoin=${URL} ../worldgreencoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoin=${COMMIT} --url worldgreencoin=${URL} ../worldgreencoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/worldgreencoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/worldgreencoin-build.assert
```
...
[worldgreencoin/gitian.sigs](https://github.com/worldgreencoin-project/gitian.sigs/) repository, or
