...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("WorldGreenCoinMiner : mined block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WorldGreenCoinMiner : ProcessBlock, block not accepted");
    }
...
bool CheckStake(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    LogPrintf("WorldGreenCoinStaker: proof-of-stake found  \n  hash: %s\n  stake: %s\n  target: %s\n", hash.GetHex(), hashStake.GetHex(), hashTarget.GetHex());
    pblock->print();
    ...
    // Found a solution
    {
        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("WorldGreenCoinStaker : minted block is stale");
        ...
        CValidationState state;
        if (!ProcessBlock(state, NULL, pblock))
            return error("WorldGreenCoinStaker : ProcessBlock, block not accepted");
    }

    return true;
}

void WorldGreenCoinStaker(CWallet *pwallet)
{
    LogPrintf("WorldGreenCoinStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the staking thread
    RenameThread("worldgreencoin-staker");
    ...
    try { while (true) {
        if (Params().NetworkID() != CChainParams::REGTEST) {
            ...
            while (vNodes.empty())
            {
                ...
                LogPrintf("WorldGreenCoinStaker : Waiting for network online.\n");
        ...
        while (IsInitialBlockDownload()) 
        {
            ...
            LogPrintf("WorldGreenCoinStaker : Waiting... Blockchain Downloading.\n");
            MilliSleep(60000);
        }

        while (pwallet->IsLocked())
        {
            LogPrintf("WorldGreenCoinStaker : Wallet is locked.\n");
            nLastCoinStakeSearchInterval = 0;
            MilliSleep(1000);
        }
        ...
        {
            LogPrintf("WorldGreenCoinStaker : Chaintip < Last POW.\n");
            MilliSleep(60000);
        }
        ...
        {
            // LogPrintf("WorldGreenCoinStaker : Failed to sign the new block.\n");
            MilliSleep(1000);
        }
    } }
    catch (boost::thread_interrupted)
    {
        LogPrintf("WorldGreenCoinStaker terminated\n");
        throw;
    }
}

void static WorldGreenCoinMiner(CWallet *pwallet)
{
    LogPrintf("WorldGreenCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("worldgreencoin-miner");
    ...
    try { while (true) {
        if (Params().NetworkID() != CChainParams::REGTEST) {
            ...
            // on an obsolete chain. In regtest mode we expect to fly solo.
            while (vNodes.empty()){
                LogPrintf("WorldGreenCoinMiner : Waiting for network online.\n");
                MilliSleep(1000);
            }
        }
        ...
        {
            // Busy-wait for the download of the blockchain to complete
            LogPrintf("WorldGreenCoinMiner : Waiting... Blockchain Downloading.\n");
            MilliSleep(60000);
        }
        ...
        {
            LogPrintf("WorldGreenCoinMiner : no more PoW blocks\n");
            return;
        }

        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        LogPrintf("Running WorldGreenCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("WorldGreenCoinMiner terminated\n");
        throw;
    }
}

void GenerateWorldGreenCoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
