...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -staking (or worldgreencoin.conf setting staking)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    if (fGenerate && Params().NetworkID() == CChainParams::REGTEST)
    {
        ...
        while (nHeight < nHeightEnd)
        {
            ...
            {
                nHeightLast = nHeight;
                GenerateWorldGreenCoins(fGenerate, pwalletMain, 1);
            }
            MilliSleep(1);
            {   // Don't keep cs_main locked
                LOCK(cs_main);
    ...
    else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateWorldGreenCoins(fGenerate, pwalletMain, nGenProcLimit);
    }
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "WorldGreenCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "WorldGreenCoin is downloading blocks...");
