...
const QString BITCOIN_IPC_PREFIX("worldgreencoin:");
const char* BITCOIN_REQUEST_MIMETYPE = "application/worldgreencoin-paymentrequest";
const char* BITCOIN_PAYMENTACK_MIMETYPE = "application/worldgreencoin-paymentack";
const char* BITCOIN_PAYMENTACK_CONTENTTYPE = "application/worldgreencoin-payment";
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start worldgreencoin: click-to-pay handler"));
        }
        else {
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
                ...
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid WorldGreenCoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
